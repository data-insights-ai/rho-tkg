...
### What's new in v4 (v4.0.0 → v4.11.1)

**v4.11.1 — hard Delete is a transaction-time tombstone in the generic TxAt door.** A hard-deleted entity vanished from every generic transaction-time read at every pin — `ByLabel`/`ByType`/`All` with `QueryOpts.TxAt`, `NodesAtTx`/`RelsAtTx`, `NodeAtTx`/`RelAtTx` — even at pins before the delete, while the named as-of door (`NodeAsOf`/`NodesAsOf`) correctly reconstructed the pre-delete belief state. Delete stamps `DeletedAt`/`ValidTo`/`TxTo` in place on the final version, and the delete-stamped `ValidTo` failed valid-time coverage in the chain resolver. Fixed at the one seam every chain-based TxAt resolution funnels through (`filterNodeChainByTxAt`/`filterRelChainByTxAt`): a surviving row whose `DeletedAt` post-dates the pin is deep-copied and normalized to the belief state as of the pin via the same `normalizeTemporalVisibleAtTxTime` the named door applies — the two doors now agree by construction, and `AS OF SYSTEM TIME` reads reproduce deleted entities at pre-delete pins. Pins at or after the delete keep excluding the entity. See `CHANGELOG.md` `[4.11.1]` and lesson 60.

**v4.11.0 — transaction-time backfill at ingest (§4.1) + named knowledge-time marks (§4.2).** Two additive, backward-compatible features so a re-ingest can reproduce a documented historical knowledge state (Erkenntniszeit) via `AS OF SYSTEM TIME`. §4.1: `TxFrom` is normally system-stamped, so loading a dataset today stamps `TxFrom = load time`. New `Config.AllowTxBackfill` (off by default — the audit/import-scope gate) lets the create doors honor a caller-supplied transaction time: `g.Nodes().AddWithTx(ctx, labels, props, txFrom)` / `g.Rels().AddWithTx(ctx, type, start, end, props, txFrom)`, or the now-caller-settable reserved property `tkg_tx_from`. Extraction lives in the one shared `extractTemporal` helper, so backfill lands uniformly across the whole create-door family (standalone node/rel `Add`+`Import`, and the `BatchBuilder` create paths, with per-entity distinct instants). It is CREATE-only — updates/deletes keep the monotonic system `TxFrom` — and `TxFrom` is not part of the integrity hash, so a backfilled row still passes `Verify*Chain` and replicates verbatim. Gate off → `ErrTxBackfillDisabled`; non-positive `txFrom` → `ErrInvalidTxFrom`. §4.2: a durable named-as-of-tag registry on `g.Temporal()` — `TagAsOf` / `ResolveAsOf` / `AsOfTags` / `RemoveAsOfTag` — maps a documented name to a transaction-time instant so `AS OF SYSTEM TIME $tag` runs against a named, stable state; stored under the `asof_tags` MetaKV key (durable across restart, reaped by `Admin().Reset()`, decoded via `SafeUnmarshal`), bounded and validated (`ErrTooManyAsOfTags` / `ErrInvalidAsOfTag`), declines without MetaKV, rejected on a read-only replica. See `CHANGELOG.md` `[4.11.0]` and lesson 59.

**v4.10.3 — deep break-rounds fix-forward.** A second adversarial campaign over the previously un-mined fuzz / crash-fault / concurrency lenses fixed two more defects, both test-first: the replica-apply / delta-merge doors now wrap `ErrCorruptExport` on a `storeutil.WireTo*Checked` field-validation failure (matching the bootstrap-import path and `verifyImported*Hash`, so `errors.Is(err, ErrCorruptExport)` classifies an untrusted-stream rejection correctly), and `PropertyKeyRegistry.GetOrCreate` now rejects blank / all-whitespace keys via `isBlankName` (matching the grow door and the label/rel-type registries). See `CHANGELOG.md` `[4.10.3]` and lesson 58.

**v4.10.2 — break-rounds fix-forward.** Two defects on the change-log / replica / delta surface, fixed test-first: delta-merge rollback no longer silently drops an untouched relationship (the `ChangeNodeHistoryVersion` / `ChangeNodeHistoryTruncate` capture branches now snapshot adjacency, so the purge-and-recreate rollback can't destroy an edge the delta never touched), and `MaxNodeHistoryID` / `MaxRelHistoryID` now agree with `AllNodeHistoryIDs` / `AllRelHistoryIDs` during the badger commit window (set-vs-delete resolved per key). See `CHANGELOG.md` `[4.10.2]` and lessons 56–57.

**v4.10.1 — per-transaction change-log buffer + change-log/replica hardening.** A rolled-back tx or batch now emits **nothing** to the op-log change-feed: the new optional `store.TxChangeLogScope` capability buffers a tx's records and mints their LSNs at commit (a rolled-back scope burns none — no feed gap), eliminating the rolled-back-token feed poison, the transient replica phantom, and the CDC asymmetry. Plus a badger commit-window overlay flush fix, a log-off `Clear()` LSN-reuse fix, and an `ApplyChanges` ascending-LSN guard. See `CHANGELOG.md` `[4.10.1]` and lessons 53–55.

**v4.10.0 — horizontal scaling: durable change-log (Phase 0) + log-shipped read replicas (Phase 1).** Opt-in `graph.Config.ChangeLog` records every committed mutation under a new `0x09` keyspace, tagged with a monotonic cluster LSN, in the SAME Badger `WriteBatch` as the data and counters (atomic; crash-consistent), surfaced via `g.Replication().ChangeFeed` / `ForEachChange` / `LastCommittedLSN` (the optional `store.ChangeFeedCapability`; memory parity via `memory.WithChangeLog()`). On top of it, a graph opened with `Config.ReadOnlyReplica` bootstraps from an export snapshot then tails a primary's change-feed to a **byte-exact** copy: `g.Replication().ApplyChange` / `ApplyChanges` reproduce the primary's rows VERBATIM through foreign-ID store doors (integrity hash, version, `TxFrom`, and temporal metadata reconstructed from the record, never re-minted), idempotent under at-least-once redelivery; `AppliedLSN` / `SetAppliedLSN` are the durable replica watermark; a core-layer read-only gate fails closed on every user mutation with the new `ErrReadOnlyReplica`. Plus gapless bootstrap via export-header `SnapshotLSN` (export format v1→v2), lazy token-registry refetch (`RegistrySnapshot` + `store.ReplicationSource` + registry `AppendNames`), and a durable ID-slot failover lease. Also in 4.10.0: **delta backups (node-level incrementals)** — `g.IO().ExportSince(w, since)` ships only the change-log mutations after a cursor, and `g.IO().ImportMerge(r, opts)` replays a delta onto a base through the same verbatim, idempotent apply doors (hash recompute-and-compare + full-chain verify, purge-and-recreate rollback); `g.IO().Watermark()` returns the current `io.Cursor{LSN, Epoch}` and `io.HeaderOf(r)` decodes a stream's `DeltaHeader`. See `CHANGELOG.md` `[4.10.0]` and `tasks/horizontal-scaling.md`.

**v4.9.4 — node property-key presence stats, streaming ForEach, bulk AddNodes.** `g.Stats().NodeCountByLabelAndPropertyKey(label, propertyKey)` is an O(1) per-`(label, property-key)` presence count over current nodes carrying an indexable scalar for the key (key-presence only, not value-selectivity — a planner can cheaply prune labels that cannot satisfy scalar-equality lookups), backed by the new optional `store.NodePropertyKeyStatsCapability`. `g.Nodes().ForEach(opts, fn)` streams current-state unpaginated scans one row at a time (O(1) peak memory; `fn` returning false stops early). `BatchBuilder.AddNodes(labels, props, count)` is a write-only bulk node create enabled by hash-suffix precomputation. Plus temporal point queries skip the version chain when the open current row answers. See `CHANGELOG.md` `[4.9.4]`.

**v4.9.3 — import framing amplification hardening.** The import trust boundary no longer lets a tiny untrusted stream amplify into a huge allocation (a memory-exhaustion DoS): each record body is read with `io.CopyN` into a growing buffer (pre-reserving at most 64 KiB) instead of `make([]byte, declaredLength)`, and the replay/rollback prealloc cap derived from the untrusted header counts dropped `1<<20` → `4096`. Both fail closed with `ErrCorruptExport`. Found by a new end-to-end `FuzzImport` harness and pinned by `runtime.TotalAlloc`-bounded regression tests. See `CHANGELOG.md` `[4.9.3]` and lesson 48.

**v4.9.2 — wire decode trust-boundary hardening.** Untrusted msgpack bytes (a corrupt disk row or a hostile import stream) can no longer crash the process at the decode boundary: `storeutil.SafeUnmarshal` runs a non-recursive depth guard that rejects container nesting beyond 64 BEFORE the decoder runs (preventing the fatal, unrecoverable stack overflow the decoder hits on a deeply-nested `PropertyWire.Value any`) and recovers the decoder's reflect panic on a duplicate interface-typed map key — both failing closed with the new `store.ErrCorruptWire` (wrapped as `ErrCorruptExport` at the import boundary). Every `NodeWire`/`RelWire`/meta decode in badger and core import now routes through it. Found by a new fuzz harness. See `CHANGELOG.md` `[4.9.2]` and lesson 47.

**v4.9.1 — X5 columnar DocValues + resident entity cache.** A per-`(label, property)` columnar value store aligned to a sorted NodeID ordinal vector: `g.Nodes().ForEachDocValues(label, propKeys, fn)` streams group keys + aggregate args from dense columns instead of fetching/decoding each node (downstream grouped 100k aggregation ~23× on badger); `ForEachDocValuesMulti(labels, …)` does the same for a label INTERSECTION; `DocValuesSnapshot(label, propKeys)` returns a random-access reader for point lookups; node/rel mutation-epoch counters are the staleness checks. Plus the resident entity cache `graph.Config.ResidentCache` keeps every decoded node/rel resident (no LRU eviction), removing the decode-on-eviction re-msgpack cost that made a working set larger than the cache scale super-linearly (opt-in, off by default). See `CHANGELOG.md` `[4.9.1]`.
...
**Snapshot-style scans exclude standalone mutations.** `g.IO().Export`, `g.Temporal().Snapshot`, and `g.Tier().VerifyShard` now hold the graph write lock for their multi-read scans, so standalone mutations cannot interleave mid-export, mid-snapshot, or mid-verification.
