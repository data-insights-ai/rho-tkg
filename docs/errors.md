...
| `ErrRelSegmentManifestInvalid` | store | The segment directory's `MANIFEST` is unreadable, damaged (magic, length, CRC32C), of an unknown version, has unknown fields, or lists something impossible (a segment of an undeclared type, a sequence at or above `next_seq`, zero rows, a bad root). The open fails closed and touches nothing (ADR-0011 S3) | `graph.New`; `OpenRelSegmentDir` |
| `ErrRelSegmentFileInvalid` | store | A segment file the manifest lists is missing, of another size than listed, damaged (a section CRC, wraps `segment.ErrCorrupt`), fails its integrity roots (wraps `segment.ErrIntegrity`, naming the `IntegrityBlockRows` groups), or does not match its manifest entry (rows, root) or its type's declaration. The open fails closed and touches nothing (ADR-0011 S3) | `graph.New`; `OpenRelSegmentDir` |
