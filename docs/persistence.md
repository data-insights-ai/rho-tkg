...
**Reading.** `g.Replication().ChangeFeed(afterLSN, limit)` returns committed records in ascending LSN order; `ForEachChange(afterLSN, fn)` streams them OOM-safe with the callback invoked outside store locks; `LastCommittedLSN()` is the watermark a session reads after a write for read-your-writes routing. Only flushed records are in the feed (`store.ChangeFeedCapability`: "a record buffered but not yet flushed is not surfaced"): on badger with `SyncWrites=false` an acknowledged write reaches the feed and `LastCommittedLSN` at the next async flush (`FlushInterval`, default 100 ms), so a replica polling right after the write sees nothing yet and gets it on a later poll; `SyncWrites=true` makes each write visible on return, and tiered flushes before every read. A backend opened without a change-log returns an empty feed (`ChangeLogEnabled()` reports `false`). All three in-tree backends — memory, badger, and (since ADR-0005 §2) tiered — implement the capability.
