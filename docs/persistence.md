...
**Deferred.** `MetaSet` does not yet emit a record (the `ChangeMeta` tag is reserved); and LSN-watermark-driven log GC (pruning records below the minimum replica-acked LSN) lands with read replicas — on tiered, a whole retention-dropped event shard drops its log segment with it, so consumers must checkpoint before a shard is retention-dropped (the same discipline any log-truncation policy requires). See `tasks/horizontal-scaling.md`.
