...
- **Relationship column doors** (`RelColumnSnapshot`, `RelMutationEpochForType`, and the `tkg_rel_start` / `tkg_rel_end` reserved column keys on the badger store) — the relationship sibling of the DocValues doors above, experimental for the same reason: the shape is pending consumer feedback. Note the rel epoch is striped per type but coarsened by any mutation site that cannot name its type, so a caller must treat the stamp as a freshness token and never as a change count
