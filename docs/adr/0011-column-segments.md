...
Status: ACCEPTED (René, 2026-09-24). S0, S1, S2 and the rho-tkg side of S5 done (2026-09-24), S3 done on the rho-tkg side (2026-09-25, see §6); S2's ai-soc parity gate, S3's BA day on Flux, the sigma release of S5 and S4 next.
Date: 2026-09-24
...
**As built (S3, 2026-09-25).** File names are `seg-t<token>-<seq>.tkgs` (the type
token, not the name: a name is not path-safe). Files are self-contained (their own
endpoint table and string dictionaries; S2's shared dictionaries stay an in-RAM
format). Order: encode → write .tmp, fsync, rename, dir fsync → map and open →
manifest commit → install. The manifest persists the declarations and is verified
at open; every listed file is matched to its entry (size, rows, root) and fully
verified (every integrity group), and only then are `.tmp` and unlisted segment
files deleted. A row the row store holds with the same ID, version and hash leaves
it; another version shadows the sealed copy. On the memory store the directory
makes the sealed rows durable, not the store: nodes, memtable, overlay and history
are process memory, so a reopen serves the committed segments' rows as sealed.
...
| **S3 — DONE 2026-09-25 (rho-tkg side); BA day on Flux open** | Segments on NVMe: mmap, manifest, crash recovery, `.tmp` cleanup. | kill at every seal step (after write, after fsync, after the manifest, before row removal); the reopened store equals the oracle | Go heap after materialize grows ≤1 B/HOP between the 790 K, 3.15 M and 12.6 M synthday days beyond the fixed budget; wall time within 10 % of S2. **Built:** `Config.SegmentDir`, `store.RelSegmentDirCapability` (memory), package `segdir` (lock, versioned CRC'd manifest, seal protocol, cleanup, mmap); files self-contained; mapped segments reference-counted; sentinels `ErrRelSegmentDirLocked` / `ErrRelSegmentManifestInvalid` / `ErrRelSegmentFileInvalid`. **Tests** (red first): SIGKILL of a child process at each of the four steps — after the segment write, after its fsync+rename, after the manifest .tmp write: the reopen serves seal A only; after the manifest is durable (before row removal) and after a completed seal: A and B — every store door equal to the oracle; injected I/O failure at each pre-commit step keeps store and directory; the S2 store suite and the graph oracle over mapped files; reopen = oracle twice with writes in between; damage fails closed (missing, truncated, CRC, a value changed with every CRC fixed → the integrity group at `IntegrityBlockRows` 1/64/4096, manifest); a scan pins its mappings across Close/Clear; `-race`. **Measured** (3 runs, 256 MiB budget, this host's NVMe, 790 K / 3.15 M / 12.6 M): Go heap beyond the memtable **0.3–0.5 / 0.3–0.4 / 0.7 B/HOP** (S2 in RAM, same runs: 24.0–24.1 / 24.7–24.9 / 25.6–25.7) — growth +0.2–0.3 B/HOP, gate met; write 12.6 M, 5 runs each alternating, pinned to 8 cores (`TestSegmentS3WriteGate`): median **7.86 s (7.85–7.88) against S2 7.87 s (7.37–7.97)**, 0 % — gate met; at an equal seal count (5) 7.86 against 7.93 s. The seal count on one input varies (4 or 5): the four budget seals run on the background sealer and each snapshots the rows written during the previous one, so boundaries move by a few thousand rows between identical runs, and a 5th (the harness's explicit final seal, after the write) exists only when rows were written after the 4th seal's snapshot; S2 and S3 count bytes identically (first snapshot 471,245–471,276 rows in both). S3's store phase per seal 40–57 ms against S2's 8–10 ms (file write 5 ms, file fsync+rename+dir fsync 14 ms, map+open+manifest write 9 ms, manifest fsync+rename+dir fsync 10 ms; ~0.1 µs/row), all on the background sealer; answers do not depend on the layout (`TestSegments_AnswersDoNotDependOnSealBoundaries`); `ScanRelSegments` 7.3–8.5 M rows/s (S2 reference 7.7–7.8); on disk 21.5 / 22.3 / 27.2–28.2 B/HOP (self-contained: per-segment node hashes 5.2 B/HOP and actor dictionaries at 4–5 segments; S4's merge bounds it); RSS at 12.6 M anon 121–132 MiB (S2 142–162), file-backed +27–28 B/HOP = the mapped segment pages (clean, reclaimable); reopen (map + verify every group) 3.3–3.5 s at 12.6 M. |
