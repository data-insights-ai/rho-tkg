...
- PERF — B6 anchor+delta version-history storage (ADR-0009), opt-in via `badger.Config.HistoryDeltaEncoding` (default OFF while it soaks). Version-history rows (badger `0x07`/`0x08`) were FULL entity snapshots — a wide entity (5–20 props, some with large unchanged values: customer/product/contact/SIEM-event) re-serialized its whole blob on every version bump. B6 keeps a full ANCHOR every 16 versions (`HistoryAnchorInterval`) and stores the rest as DELTAS carrying only the properties that CHANGED vs the interval anchor (plus both hashes + the full temporal block verbatim). **MEASURED ~39% less history storage post-block-Snappy** on history-heavy wide entities (the Phase 0.9 gate `wire_b6_history_gate_test.go`; the companion B3 timestamp-delta lever was DROPPED at 1.13% post-Snappy — see lessons 67). Design:
