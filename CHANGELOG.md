...
- ADD — `g.Stats().RelPropertyStats(typeName, propKey)` — the relationship mirror of node `PropertyStats` (BACKLOG 21a), completing the rel-side ordering-soundness/planner-stats trio alongside `RelRangeCardinality` and `RelPropertyTypeClassCounts`. Returns the same `store.PropertyStats{NDV, Min, Max, Count}` shape (HyperLogLog NDV estimate + exact min/max over scalar-ordered value families + the presence count), keyed by `(relTypeToken, propertyKey)` instead of `(labelToken, propertyKey)` — a query planner costing a relationship-property predicate previously had count/type-class tools (`RelCountByType`, `RelPropertyTypeClassCounts`) but no selectivity estimate. New rel-side infrastructure (there was none):
  - New optional `store.RelPropertyStatsCapability` (native memory + badger). Tiered DECLINES (`ErrCapabilityNotSupported`), matching the precedent already set by its two rel-side siblings `RelRangeCardinality`/`RelPropertyTypeClassCounts` — neither of which tiered implements either, since rel property indexes are RAM-only per-shard with no cross-shard fold defined. `DisablePlannerStats` declines too (parity with the node stat opt-out).
  - **The delete-without-properties sharp edge, solved the same way as 5B.** Badger's `deleteRelByInfo` is read-free (`RelDeleteInfo` carries no property values), so a NEW, SEPARATE memoized-contribution sidecar (`relStatsContrib`, keyed by rel ID, populated at every ADD) lets the single delete seam `Forget()` the NDV+min/max accumulator precisely by ID. It cannot reuse the existing `relTypeClassContrib` sidecar from 5B: `types.Relationship.ForEachPropertyTypeClass` deliberately never exposes the raw value (key + class only), so a parallel `relStatsEntry{key, valueKey, value}` memoization was needed. The memory store holds live rels, so it decrements with the old rel directly (no sidecar needed there — mirroring `relPropertyTypeClassCounts`'s existing shape). Counters + sidecar are co-located with the rel property-index and type-class maintenance at every full-rel-write ADD/REPLACE/history/batch site and REBUILT from `loadIndexes` at open (the "survives restart" rule).
  - Locking on badger follows the SAME fine-grained, write-generation-guarded dirty-Rescan discipline as `NodePropertyStats` (a cache-cold `prefetchRelScan` needs its own brief `idxMu.RLock`, so the rescan collect runs with `idxMu` not held for the whole call); memory holds one lock for the whole call (no re-entrant-locking risk there).
  - No `RelPropertyStatsSketch` internal accessor was added — unlike its node-side counterpart `NodePropertyStatsSketch`, it would have zero callers: tiered's cross-shard sketch-merge fold is the sketch accessor's only consumer, and tiered does not implement `RelPropertyStats` (see above).
  - Wiring: badger `RelPropertyStats` + `addRelPropertyStatsCounts`/`removeRelPropertyStatsCountsByID`/`adjustRelPropertyStatsOne`; memory `adjustRelPropertyKeyCounts`/`adjustRelPropertyStatsOne`; core `StatOps.RelPropertyStats` (capability checked BEFORE the rel-type-token lookup, so tiered's decline is deterministic even for a never-used type name) + the public `g.Stats()` wrapper. Tests (`pkg/graph/stats_rel_propertystats_test.go`, both backends): basic NDV/min/max/count, unregistered-pair zero value, non-indexable value excluded, delete-the-extremum triggers a min/max rescan (NDV never decrements — a HyperLogLog property), an Update changing a rel's property value swaps the old/new contribution, node-cascade-delete drains the counters to zero (the memoized-sidecar seam, no drift), badger reopen (counters + sidecar rebuild, post-reopen delete still decrements + rescans correctly), tiered decline, and an NDV-accuracy cross-check (200 distinct values, wide-stride spread, within ±25% of true cardinality); `-race` clean.
  - Out-of-scope finding, noted for a future ticket: the NDV-accuracy test needed widely-spread values rather than small sequential integers — `internal/index.HyperLogLog.AddString`'s FNV-1a hash has poor top-bit avalanche for the near-identical short strings `IndexablePropertyValueKey` produces for a run of small sequential integers (e.g. `"i64:0"`..`"i64:199"`), which collapsed 200 distinct values into ~17 register buckets. This is a pre-existing characteristic of the ALREADY-SHIPPED shared sketch (equally affects `NodePropertyStats`), not something introduced here, and fixing a hash-quality issue in shared `internal/index` infrastructure was out of scope for this rel-side-mirror change.
