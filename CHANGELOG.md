...
- **TieredStore empty-history fan-out no longer wakes cold shards** (`pkg/graph/tieredstore_read.go`, `pkg/graph/tieredstore_write.go`): `GetNodeHistory`, `GetRelHistory`, `GetNodeVersion`, `GetRelVersion`, `TruncateNodeHistory`, and `TruncateRelHistory` skip the cross-shard fan-out when the live entity is present on its home shard. The fan-out is only needed to recover history of a deleted entity whose home shard no longer holds the live index — for an alive entity with no history, the empty result is authoritative locally. Exception: when the home shard is `refArchive`, the fan-out still runs because `ArchiveNode` only migrates the live entity and pre-archive history versions remain on `refShard`. Avoids needlessly opening cold shards for operations on freshly created or never-versioned entities while preserving correctness for archived ref nodes.
- **TieredStore checkout pinning on rel write paths and node-keyed adjacency reads** (`pkg/graph/tieredstore_read.go`, `pkg/graph/tieredstore_write.go`): `ReplaceRelationship`, `DeleteRelationship`, `DeleteRelWithHistory`, `OutgoingRelationships`, `OutgoingRelationshipsForNodes`, `IncomingRelationships`, and `IncomingRelationshipsForNodes` now resolve their owner shards via `shardForRelIDChecked`/`shardForNodeIDChecked` and `defer checkin()`. Previously they used the unchecked variants and dereferenced the returned `*BadgerStore` pointer — once the rel-fallback probe started opening cold shards, that pointer could be closed by `closeIdleShards` mid-operation. Closes the cold-shard close race for the rel write path and the matching node-keyed adjacency reads.
- **TieredStore checkout pinning on remaining write paths** (`pkg/graph/tieredstore_write.go`): `PutRelationship`, `PutRelationshipsBatch`, `DeleteNodesBatch`, `DeleteNodeCascade`, `DeleteNodeWithHistory`, `ReplaceNodeWithHistory`, `ReplaceRelWithHistory`, `PutNodeVersion`, and `PutRelVersion` now also resolve their owner shards via the checked resolvers and pin the cold owner for the duration of the write. Closes the same idle-close race on update / version-history / cascade-delete paths that hand a `*BadgerStore` to per-shard write logic.
- **TieredStore relationship resolver probes refArchive** (`pkg/graph/tieredstore.go`): `shardForRelID` and `shardForRelIDChecked` now check `refArchive` (between `refShard` and the timestamp candidate) so any rel that ends up on the archive — e.g. when both endpoints get archived together by `ArchiveNode` — remains reachable through every public read/write path. Mirrors the equivalent `shardForNodeID(Checked)` archive probe. The empty-history skip on `GetRelHistory`/`GetRelVersion`/`TruncateRelHistory` now also requires `shard != refArchive` so pre-archive rel history (which stays on `refShard`) is still surfaced via the fan-out for archived rels.
- **`ReplaceRelWithHistory` routes by relationship ID, not start-node ID** (`pkg/graph/tieredstore_write.go`): every other rel-write path resolves the owner via `shardForRelIDChecked(rel ID)`; this method previously used `shardForNodeIDChecked(StartNodeID)`. For rels whose entity has been migrated independently of the start node (e.g. archived together with both endpoints) the start-node-keyed lookup picks the wrong shard and skips the `refArchive` probe baked into `shardForRelIDChecked`. Closes the last remaining outlier in the rel-resolver audit.
- **`DeleteRelationship` cross-shard rollback parity** (`pkg/graph/tieredstore_write.go`): the cross-shard delete previously deleted the entity+out from the entity shard then attempted the in/ delete on the end-node shard with no rollback — a failure of the second leg would leave a phantom incoming-index entry. Now mirrors the rollback already in place on the create path: on second-leg failure the entity+out write is restored via `putRelEntityAndOut(r)` so a re-read still observes the rel.
- **`ensureRefArchive` ↔ `Close` race** (`pkg/graph/tieredstore.go`, new `ErrStoreClosed` sentinel): a reader that observed `refArchive==nil` could lazy-open a fresh archive after `Close` had already closed the store, leaking the freshly opened DB handle. `Close` now sets a `closed atomic.Bool` under `archiveMu` before tearing the archive down; `ensureRefArchive` consults the flag (also under `archiveMu`) and returns `ErrStoreClosed` instead of opening. The lock makes the close-vs-open transition a single critical section.
...
- **TieredStore history-routing regression tests** (`pkg/graph/tieredstore_history_routing_test.go`): direct coverage for `shardForRelIDChecked` on same-shard reference rels and unknown IDs; rejection of primary-label-class change via `RemoveNodeLabel`; allowed same-class label add/remove; `GetNodeVersion`/`GetRelVersion` after deleting a reference node and a cross-shard relationship; `TruncateNodeHistory` after deleting a reference node; silent no-op truncate semantics for unknown IDs; `GetRelHistory` on a cross-shard rel created after rotation whose start-node shard later transitions warm→cold (regression guard for `forEachHistoryShard` cold-shard exclusion); `TestTieredStore_PublicRelationshipReads_LivePostRotationRelAfterStartShardCold` covers `GetRelationship`/`OutgoingRelationships`/`IncomingRelationships`/`IncomingRelationshipsForNodes` after the start-node shard ages to cold; `TestTieredStore_ShardForRelIDChecked_LiveColdRelPinsShardDuringRead` asserts that fallback to a cold relationship owner pins the shard until checkin so idle-close cannot close it during a read; `TestTieredStore_EmptyHistoryLookups_DoNotOpenColdShards` asserts that empty-history history-read/truncate paths do not lazy-open cold shards when the live entity sits on its home shard; `TestTieredStore_ArchivedNode_HistorySurvives` asserts `GetNodeHistory`/`GetNodeVersion`/`TruncateNodeHistory` still surface pre-archive history after `ArchiveNode` migrates the live entity to `refArchive`; `TestTieredStore_DeleteRelationship_CrossShardKeepsCheckoutAlive` exercises the new checked-resolution code path on a cross-shard delete after cold demotion; `TestTieredStore_RelMutations_AfterStartShardCold` covers `Graph.UpdateRelationship` and `Graph.DeleteRelationship` end-to-end on a live post-rotation E→E rel whose start-node shard has been demoted to cold (the cycle that previously raced `closeIdleShards` against `ReplaceRelWithHistory`/`DeleteRelWithHistory`).
- **TieredStore `shardForRelID` cold-shard probe** (`pkg/graph/tieredstore_test.go`): replaces the obsolete `TestTieredStore_ShardForRelID_SkipsColdShards` (which encoded the now-incorrect skip-cold optimization) with `TestTieredStore_ShardForRelID_FindsRelOnColdShard`, asserting the lookup finds a cross-shard rel whose start shard has been demoted to cold.
