...
| `temporal.go` | `GraphSnapshot` struct + temporal query methods: `GetNodesValidAt`, `GetRelationshipsValidAt`, `GetNodesByLabelValidAt`, `GetNodesValidDuring`, `GetRelationshipsValidDuring`, `GetNodeAt`, `GetRelAt`, `GetNeighborsValidAt`, `Snapshot`. Combined queries: `NodesByLabelPropertyAndTime(label, key, value, t)`, `NodesByLabelPropertyDuring(label, key, value, start, end)` — compose property index with temporal filter. History-aware: merges current + historical entity IDs via `AllNodeHistoryIDs`/`AllRelHistoryIDs`, handles deleted entities (nil current). Internal helpers: `nodeValidFrom`/`relValidFrom`, `nodeVersionBounds`/`relVersionBounds`, `resolveNodeVersionAt`/`resolveRelVersionAt`, `allKnownNodeIDs`/`allKnownRelIDs`, `getNodeVersionDuring`/`getRelVersionDuring` |
