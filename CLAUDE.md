...
| `pkg/graph/store` | — | `Store` interface, `QueryOpts`, `ShardDepth`, `RelTombstone`, `DistanceMetric`, `ChangeFeedCapability` + `ChangeRecord`/`ChangeTag` (optional op-log), `ChangeLogStatusCapability` (`ChangeLogEnabled()` — recording vs present-but-off), `TxChangeLogScope` (per-tx record buffer: a rolled-back tx/batch emits nothing), `ReplicationSource`/`RegistrySnapshot`/`IDSlotLeaseRecord` (replica refetch + failover), 30 store sentinels. |
