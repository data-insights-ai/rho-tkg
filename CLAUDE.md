...
| `store.go` | `Store` interface (pure persistence contract with error-returning query methods, bulk queries, batch operations (`PutNodesBatch`, `PutRelationshipsBatch`, `DeleteNodesBatch`, `DeleteRelationshipsBatch`), atomic replace+history (`ReplaceNodeWithHistory`, `ReplaceRelWithHistory`), `DeleteNodeCascade`, `Close()` for resource cleanup, 8 version history methods) + sentinel errors (`ErrNodeNotFound`, `ErrRelNotFound`, `ErrNodeExists`, `ErrRelExists`, `ErrVersionNotFound`) |
