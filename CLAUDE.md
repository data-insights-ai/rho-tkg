...
| `pkg/graph/store` | — | `Store` interface, `QueryOpts`, `ShardDepth`, `RelTombstone`, `DistanceMetric`, `ChangeFeedCapability` + `ChangeRecord`/`ChangeTag` (optional op-log), `ReplicationSource`/`RegistrySnapshot`/`IDSlotLeaseRecord` (replica refetch + failover), 30 store sentinels. |
