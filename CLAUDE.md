...
- **Inside a tx, use tx-side read mirrors**: `BeginTx` holds `c.mu.Lock` for the entire transaction. Calling any public read accessor that opens with `c.mu.RLock` (`g.Nodes.Labels`/`HasLabel`/`PrimaryLabel`, `g.Rels.Type`/`HasType`, `g.Resolve.NodeProperty`/`RelProperty`, plus `g.IO.Export`, `g.Temporal.Snapshot`, `g.Tier.VerifyShard`) deadlocks the calling goroutine. Use the `*GraphTx` mirrors in `pkg/graph/internal/core/tx_consistent_reads.go` — they call the lock-free `*Unlocked` helpers directly under `tx.mu`. Whenever you add a new RLock-guarded public read accessor, you owe a matching tx mirror. See lessons.md #31.
