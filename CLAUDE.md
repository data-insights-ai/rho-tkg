...
| `pkg/graph/store` | — | `Store` interface, `QueryOpts`, `ShardDepth`, `RelTombstone`, `DistanceMetric`, `ChangeFeedCapability` + `ChangeRecord`/`ChangeTag` (optional op-log), `ChangeLogStatusCapability` (`ChangeLogEnabled()` — distinguishes recording from present-but-off; used by `ExportSince`/`Watermark`), 12 store sentinels. |
