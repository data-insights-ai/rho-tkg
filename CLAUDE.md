...
| `tieredstore.go` | `TieredStoreConfig`, `TieredStore` (Store impl), `eventShard` — routes entities across ref shard + time-windowed event shards by ontology classification. `mu sync.RWMutex` protects hotShard pointer and eventShards map during rotation. Snowflake ID timestamp extraction for O(1) shard resolution. `RotateHotShard()` demotes hot→warm (flush, mark read-only, create new hot shard with ms-aligned boundaries). `checkRotation()` fast-path time check + slow-path Lock+double-check+rotate. `eventShardSnapshot(depth)` returns depth-filtered shard stores. Warm shard recovery in constructor (reopens warm shards from catalog as ReadOnly). `shardForRelID` probes all event shards for cross-shard relationship entities. Directory layout: `data/meta/`, `data/reference/`, `data/events/<window>/`. Mid-window restart via catalog. `Close()` with `sync.Once` |
...
| `tieredstore_read.go` | TieredStore read operations — ref probe + timestamp fallback (O(1), no fan-out). Merge queries (AllNodes, AllRels, AllNodeIDs, AllRelIDs, counts, history IDs) use `eventShardSnapshot(opts.Depth)` under `mu.RLock` for depth-aware shard filtering. k-way merge of sorted slices. `IncomingRelationships` cross-shard: get relIDs from node's shard inIdx, fetch each entity via `shardForRelID`. Pagination on merged results |
