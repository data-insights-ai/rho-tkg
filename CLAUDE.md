...
Status: v3.0.31
Phases complete: 1a-1g, 2a-2i, 3a-3e
Bug fixes: v3.0.23 (6 issues), v3.0.26 (3 concurrency/integrity bugs), v3.0.30 (5 bugs — checkout/checkin race, cold shard skip, archive rollback, dirty-map tracking, canonical hash), v3.0.31 (OOM fix — lazy ForEach iterators for temporal pipeline, ~83% memory reduction)
...
| `temporal.go` | `GraphSnapshot` struct + temporal query methods: `GetNodesValidAt`, `GetRelationshipsValidAt`, `GetNodesByLabelValidAt`, `GetNodesValidDuring`, `GetRelationshipsValidDuring`, `GetNodeAt`, `GetRelAt`, `GetNeighborsValidAt`, `Snapshot`. Combined queries: `NodesByLabelPropertyAndTime(label, key, value, t)`, `NodesByLabelPropertyDuring(label, key, value, start, end)` — push temporal filters into Store via `QueryOpts`. History-aware: merges current + historical entity IDs via lazy `ForEachNodeID`/`ForEachNodeHistoryID`/`ForEachRelID`/`ForEachRelHistoryID` iterators (no slice materialization), handles deleted entities (nil current). Internal helpers: `nodeValidFrom`/`relValidFrom`, `nodeVersionBounds`/`relVersionBounds`, `resolveNodeVersionAt`/`resolveRelVersionAt`, `forEachKnownNodeID`/`forEachKnownRelID` (two-phase: collect IDs under store lock, process after release), `getNodeVersionDuring`/`getRelVersionDuring` |
...
| `tieredstore_read.go` | TieredStore read operations — ref probe + archive fallback + timestamp fallback (O(1), no fan-out). Merge queries (AllNodes, AllRels, AllNodeIDs, AllRelIDs, counts, history IDs) use `eventShardSnapshot(opts.Depth)` under `mu.RLock`, parallel via `sync.WaitGroup` (ref shard sequential, event shards concurrent). ForEach iterators (`ForEachNodeID`, `ForEachRelID`, `ForEachNodeHistoryID`, `ForEachRelHistoryID`) use sequential shard iteration with checkout/checkin — one shard open at a time, no goroutines, no `mergeIDSlices`. `es.getStore(ts)` for lazy-open of cold shards. `stripDepth` helper. k-way merge of sorted slices. `IncomingRelationships` cross-shard: get relIDs from node's shard inIdx, fetch each entity via `shardForRelID`. Pagination on merged results |
...
**Store is pure persistence**: The `Store` interface handles entity CRUD, index maintenance, atomic cascade operations, per-label/per-type counting, `Clear()`, and `Close()`. Shadow resolution and string resolution are Graph-layer responsibilities — never Store. All query methods return `error` and sort by snowflake.ID for determinism. Five paginated queries accept `QueryOpts` (cursor + temporal push-down); zero values = "return all / no filter". See the architecture table above for store-specific implementation details (MemoryStore, BadgerStore, TieredStore).
...
**API design rules**: Config fields must be used or removed — never accept input that does nothing. Opaque wrappers must wrap the real type (`type nodeID snowflake.ID`, not `int64`). Graph is the sole external API — add passthroughs rather than exposing Store. Doc comments must match behavior — after changing logic, grep for stale descriptions.
