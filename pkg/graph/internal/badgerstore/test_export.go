...
func (bs *BadgerStore) SetDBClosedForTest(closed bool) { bs.dbClosed.Store(closed) }
...
func (bs *BadgerStore) SetNodeCountForTest(n int64) { bs.nodeCount.Store(n) }
...
//
// Exported solely for test use; not for production.
func (bs *BadgerStore) LockFlushMuForTest()   { bs.flushMu.Lock() }
func (bs *BadgerStore) UnlockFlushMuForTest() { bs.flushMu.Unlock() }

// DBForTest returns the underlying *badger.DB so corruption-injection tests in
// pkg/graph can write malformed entries directly. Not for production use.
func (bs *BadgerStore) DBForTest() *badger.DB { return bs.db }
...
func (bs *BadgerStore) NodeCacheForTest() *indexpkg.Cache[*types.Node]        { return bs.nodeCache }
func (bs *BadgerStore) RelCacheForTest() *indexpkg.Cache[*types.Relationship] { return bs.relCache }
...
// the BadgerStore lock. Returns (nil, false) if the label token is unknown.
//
...
func (bs *BadgerStore) LabelIndexForTest(token uint16) ([]types.NodeID, bool) {
	bs.idxMu.RLock()
...
func (bs *BadgerStore) HasNodeIDForTest(id snowflake.ID) bool {
	bs.idxMu.RLock()
	defer bs.idxMu.RUnlock()
...
func (bs *BadgerStore) HasRelIDForTest(id snowflake.ID) bool {
	bs.idxMu.RLock()
	defer bs.idxMu.RUnlock()
...
func (bs *BadgerStore) SyncWritesForTest() bool { return bs.syncWrites }
...
func (bs *BadgerStore) FlushIntervalForTest() time.Duration { return bs.flushInt }
...
// CreateTemporalIndex/DropTemporalIndex propagated to a specific BadgerStore
// instance (e.g. the refArchive shard inside TieredStore).
func (bs *BadgerStore) HasTemporalIndexForTest(token uint16) bool {
	bs.idxMu.RLock()
	defer bs.idxMu.RUnlock()
...
func (bs *BadgerStore) HasHFIndexForTest(token uint16) bool {
	bs.idxMu.RLock()
	defer bs.idxMu.RUnlock()
...
func (bs *BadgerStore) LockIdxMuRForTest()   { bs.idxMu.RLock() }
func (bs *BadgerStore) UnlockIdxMuRForTest() { bs.idxMu.RUnlock() }

// ReadOnlyForTest reports whether the BadgerStore was opened read-only.
// Exposed so tests can assert tier-state plumbing through TieredStore.
func (bs *BadgerStore) ReadOnlyForTest() bool { return bs.readOnly }

// FlushDoneForTest / GCDoneForTest return the lifecycle done-channels for
// tests that wait on goroutine shutdown.
func (bs *BadgerStore) FlushDoneForTest() <-chan struct{} { return bs.flushDone }
func (bs *BadgerStore) GCDoneForTest() <-chan struct{}    { return bs.gcDone }
