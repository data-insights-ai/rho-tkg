// Package store hosts the persistence-contract types (Store interface,
// QueryOpts, sentinel errors), the binary key encoding used by the Badger
// backend, the msgpack wire format used by all backends, and the
// snowflake-layout helpers shared by every package in pkg/graph.
//
// This package is internal to pkg/graph. Identifiers are exported only
// because they cross package boundaries inside pkg/graph; the public
// graph API still lives in the parent pkg/graph package and re-exports
// these types via aliases.
package store
