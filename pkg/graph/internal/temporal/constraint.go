// Package temporal contains pure-data types for graph temporal-constraint
// configuration. The Graph-coupled enforcement logic (checkTemporalConstraints
// and friends) lives in pkg/graph; the symbols here are re-exported from
// pkg/graph/aliases.go so the public API surface is unchanged.
package temporal
