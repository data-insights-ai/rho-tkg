...
// MemoryStore read lock for the duration of the lookup.
//
...
func (ms *MemoryStore) GetNodeHistoryEntry(id types.NodeID, version uint32) *types.Node {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *MemoryStore) SetNodeHistoryEntryForTest(id types.NodeID, version uint32, n *types.Node) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	hist, ok := ms.nodeHistory[id]
	if !ok {
...
func (ms *MemoryStore) SetNodeForTest(id types.NodeID, n *types.Node) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
