...
func propertiesToWire(ps types.PropertySlice) []propertyWire {
	...
	for i, p := range ps {
		pw[i] = propertyWire{Key: p.Key, Value: p.Value}
	}
	return pw
}
...
//
// Msgpack type normalization: integers decode as int64/uint64, floats as float64,
// slices as []any, maps as map[string]any. All of these are on the PropertySlice
// allowlist, so no post-decode conversion is needed.
func wireToProperties(pw []propertyWire) types.PropertySlice {
	...
	for i, p := range pw {
		ps[i] = types.Property{Key: p.Key, Value: p.Value}
	}
	return ps
}
