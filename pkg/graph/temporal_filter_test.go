...
// makeIDWithTime creates a snowflake ID with the given time offset (ms from epoch).
// Layout: time(42b) | node(10b) | step(12b).
func makeIDWithTime(timeMs int64) snowflake.ID {
	return snowflake.ID(timeMs << 22)
}

func TestEntityValidFrom_ExplicitValidFrom(t *testing.T) {
	t.Parallel()
	id := makeIDWithTime(5000)
...
func TestEntityValidFrom_DerivedFromSnowflake(t *testing.T) {
	t.Parallel()
	timeMs := int64(12345)
	id := makeIDWithTime(timeMs)
	got := entityValidFrom(id, nil)
	want := types.Instant(snowflakeEpoch.UnixMilli() + timeMs)
	if got != want {
...
func TestMatchesTemporalFilter_ValidAtMatch(t *testing.T) {
	...
	id := makeIDWithTime(0) // derived time irrelevant because ValidFrom is explicit
