...
func TestBatchCreation_UsesSharedMetadataPreparation(t *testing.T) {
	...
	storedRel, err := g.GetRelationship(r.ID())
	if err != nil {
...
func TestBatchCreation_StampsMetadataAtExecuteTime(t *testing.T) {
	...
	updatedA, err := g.UpdateNode(a.ID(), map[string]any{"v": int64(2)})
	if err != nil {
	...
	storedR, err := g.GetRelationship(r.ID())
	if err != nil {
