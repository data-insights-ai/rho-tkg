...
func (g *Graph) SearchNearestNodes(label, propertyKey string, query []float32, k int, opts QueryOpts) ([]*types.Node, error) {
	...
	filter := func(id snowflake.ID) bool {
		_, err := g.findNodeVersionForOpts(types.NodeID(id), opts, pred)
		return err == nil
	}

	store, ok := g.store.(filteredVectorSearchStore)
	if !ok {
	...
	for _, id := range ids {
		n, err := g.findNodeVersionForOpts(types.NodeID(id), opts, pred)
		if err != nil {
