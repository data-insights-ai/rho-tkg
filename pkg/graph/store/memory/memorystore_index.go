...
func (ms *Store) CreateVectorIndex(labelToken uint16, propertyKey string, dims int, metric DistanceMetric) error {
	...
	for id, n := range ms.nodes {
		...
		if err := vi.Add(id.SnowflakeID(), vec); err != nil {
...
func (ms *Store) SearchNearestNodes(labelToken uint16, propertyKey string, query []float32, k int, opts QueryOpts) ([]*types.Node, error) {
	...
	filter, err := ms.vectorTemporalFilter(vi, opts)
	if err != nil {
		return nil, err
	}
...
func (ms *Store) vectorTemporalFilter(vi *indexpkg.VectorIndex, opts QueryOpts) (func(snowflake.ID) bool, error) {
	if !storepkg.HasTemporalFilter(opts) {
		return nil, nil
	}
	eligible := make(map[snowflake.ID]struct{})
	for _, id := range vi.IDs() {
		n, err := ms.GetNode(types.NodeID(id))
		if err != nil {
