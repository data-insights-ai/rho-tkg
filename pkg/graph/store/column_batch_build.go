...
func ScanColumnsFromNodes(nodes []*types.Node, props []string,
	...
	for _, n := range nodes {
		...
		if err := batch.appendRow(props, kindKnown, n.GetProperty); err != nil {
			return err
		}
...
func ScanColumnsFromRels(rels []*types.Relationship, props []string,
	...
	for _, r := range rels {
		...
		batch.ValidFrom = append(batch.ValidFrom, int64(vf))
		batch.ValidTo = append(batch.ValidTo, int64(vt))
		if err := batch.appendRow(props, kindKnown, r.GetProperty); err != nil {
			return err
		}
		if len(batch.IDs) >= ColumnScanBatchRows {
			if !fn(batch) {
...
// appendRow appends one entity's values across every requested column.
//
// This is the single implementation of the kind-resolution rules, called by both
// the node and the relationship drivers. The COLUMN KIND is decided by the first
// non-absent value in each column and every later value must convert to it; a value
// that does not is treated as absent rather than coerced, because silently reading
// a string column as zero would be worse than reporting the row as missing.
func (cd *ColumnData) appendRow(props []string, kindKnown []bool,
	getProp func(string) (any, bool)) error {

	for c, key := range props {
		raw, found := getProp(key)
