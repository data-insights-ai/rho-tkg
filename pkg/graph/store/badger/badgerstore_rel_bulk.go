...
//     Seek, almost all of it discarded. Do not flip it (set in scanSnapshot).
//
// Misses read through a scanSnapshot, which re-pins after a flush: see its doc
// comment for the dropped or replaced row a single up-front snapshot causes.
//
...
func (bs *Store) forEachRelBulk(ids []types.RelID, fn func(*types.Relationship) bool) error {
	...
	for _, rid := range ids {
		...
		item := snap.seekAfterMiss(storepkg.RelKey(id))
		if item == nil {
