...
func TestPutRelEntityAndOut_CreatesEntityButNotInIdx(t *testing.T) {
	...
	n2 := types.NewNode(gen.Generate(), 1, nil)
	...
	r := types.NewRelationship(relID, 1, n1.InternalID().SnowflakeID(), n2.InternalID().SnowflakeID())
	...
	got, err := bs.GetRelationship(relID)
	if err != nil {
	...
	outIDs := bs.outgoingRelIDs(n1.InternalID().SnowflakeID())
	...
	inIDs := bs.incomingRelIDs(n2.InternalID().SnowflakeID(), 0)
	if len(inIDs) != 0 {
...
func TestPutRelIncoming_CreatesInIdxOnly(t *testing.T) {
	...
	gen := newTestGen(t, 0)
	endNode := types.NewNode(gen.Generate(), 1, nil)
	...
	if err := bs.putRelIncoming(endNode.InternalID().SnowflakeID(), startID, relType, relID); err != nil {
		t.Fatal(err)
	}

	// inIdx should contain the rel.
	inIDs := bs.incomingRelIDs(endNode.InternalID().SnowflakeID(), 0)
...
func TestDeleteRelEntityAndOut_RemovesEntityButNotInIdx(t *testing.T) {
	...
	if info.startID != n1.InternalID().SnowflakeID() {
		t.Error("info.startID mismatch")
	}
	if info.endID != n2.InternalID().SnowflakeID() {
...
func TestIncomingRelIDs_TypeFilter(t *testing.T) {
	...
	// Type 1 rel.
	r1 := types.NewRelationship(relGen.Generate(), 1, n1.InternalID().SnowflakeID(), n2.InternalID().SnowflakeID())
	if err := bs.PutRelationship(r1); err != nil {
		t.Fatal(err)
	}

	// Type 2 rel.
	r2 := types.NewRelationship(relGen.Generate(), 2, n1.InternalID().SnowflakeID(), n2.InternalID().SnowflakeID())
	if err := bs.PutRelationship(r2); err != nil {
		t.Fatal(err)
	}

	// All types.
	all := bs.incomingRelIDs(n2.InternalID().SnowflakeID(), 0)
	if len(all) != 2 {
		t.Fatalf("incomingRelIDs(0) = %d, want 2", len(all))
	}

	// Filter type 1.
	type1 := bs.incomingRelIDs(n2.InternalID().SnowflakeID(), 1)
	if len(type1) != 1 {
		t.Fatalf("incomingRelIDs(1) = %d, want 1", len(type1))
	}

	// Filter type 2.
	type2 := bs.incomingRelIDs(n2.InternalID().SnowflakeID(), 2)
	if len(type2) != 1 {
		t.Fatalf("incomingRelIDs(2) = %d, want 1", len(type2))
	}

	// Filter unknown type.
	type99 := bs.incomingRelIDs(n2.InternalID().SnowflakeID(), 99)
	if len(type99) != 0 {
