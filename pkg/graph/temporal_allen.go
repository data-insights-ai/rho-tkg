...
func (g *Graph) NodeInterval(n *types.Node) (start, end types.Instant, err error) {
...
//
// Returns types.ErrOpenInterval if ValidTo == 0 (open-ended / still valid).
func (g *Graph) RelInterval(r *types.Relationship) (start, end types.Instant, err error) {
...
func (g *Graph) RelateNodes(a, b *types.Node) (types.AllenRelation, error) {
	aStart, aEnd, err := g.NodeInterval(a)
	if err != nil {
		return 0, err
	}
	bStart, bEnd, err := g.NodeInterval(b)
	if err != nil {
		return 0, err
	}
...
func (g *Graph) RelateRels(a, b *types.Relationship) (types.AllenRelation, error) {
	aStart, aEnd, err := g.RelInterval(a)
	if err != nil {
		return 0, err
	}
	bStart, bEnd, err := g.RelInterval(b)
	if err != nil {
		return 0, err
	}
	return types.Relate(aStart, aEnd, bStart, bEnd)
