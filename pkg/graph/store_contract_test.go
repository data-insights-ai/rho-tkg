	...
	return []storeContractBackend{
		{
			...
			newGraph: func(t *testing.T, snowflakeNodeID int) *Graph {
				t.Helper()
				g, err := New(Config{SnowflakeNodeID: int64(snowflakeNodeID), Store: NewMemoryStore()})
				if err != nil {
...
func TestStoreContract_CurrentVisibility(t *testing.T) {
	runStoreContract(t, "current visibility", func(t *testing.T, backend storeContractBackend) {
		...
		assertNodeIDs(t, "AllNodes", mustAllNodes(t, g, QueryOpts{}),
			[]snowflake.ID{caseNode.InternalID().SnowflakeID(), signalNode.InternalID().SnowflakeID()})
		assertNodeIDs(t, "NodesByLabel(Case)", mustNodesByLabel(t, g, "Case", QueryOpts{}),
			[]snowflake.ID{caseNode.InternalID().SnowflakeID()})
		assertNodeIDs(t, "NodesByLabel(Signal)", mustNodesByLabel(t, g, "Signal", QueryOpts{}),
			[]snowflake.ID{signalNode.InternalID().SnowflakeID()})

		assertRelIDs(t, "AllRelationships", mustAllRelationships(t, g, QueryOpts{}),
			[]snowflake.ID{observed.InternalID().SnowflakeID()})
		assertRelIDs(t, "RelationshipsByType(OBSERVED)", mustRelationshipsByType(t, g, "OBSERVED", QueryOpts{}),
			[]snowflake.ID{observed.InternalID().SnowflakeID()})
		assertRelIDs(t, "OutgoingRelationships(Case)", mustOutgoingRelationships(t, g, caseNode.InternalID().SnowflakeID(), ""),
			[]snowflake.ID{observed.InternalID().SnowflakeID()})
		assertRelIDs(t, "IncomingRelationships(Signal)", mustIncomingRelationships(t, g, signalNode.InternalID().SnowflakeID(), ""),
			[]snowflake.ID{observed.InternalID().SnowflakeID()})
	})
}
...
func TestStoreContract_HistoryVisibility(t *testing.T) {
	runStoreContract(t, "history visibility", func(t *testing.T, backend storeContractBackend) {
		...
		updatedNode, err := g.UpdateNode(caseNode.InternalID().SnowflakeID(), map[string]any{"status": "investigating"})
		if err != nil {
		...
		updatedRel, err := g.UpdateRelationship(rel.InternalID().SnowflakeID(), map[string]any{"weight": int64(2)})
		if err != nil {
		...
		nodeHistory, err := g.GetNodeHistory(caseNode.InternalID().SnowflakeID())
		if err != nil {
		...
		relHistory, err := g.GetRelHistory(rel.InternalID().SnowflakeID())
		if err != nil {
		...
		assertRelIDs(t, "AllRelHistoryIDs", idsToRels(t, g.store, mustStoreRelHistoryIDs(t, g.store)),
			[]snowflake.ID{rel.InternalID().SnowflakeID()})

		gotCurrent, err := g.GetNode(caseNode.InternalID().SnowflakeID())
		if err != nil {
		...
		gotSignal, err := g.GetNode(signalNode.InternalID().SnowflakeID())
		if err != nil {
...
func TestStoreContract_DeleteTombstonesAndHistory(t *testing.T) {
	runStoreContract(t, "delete tombstones and history", func(t *testing.T, backend storeContractBackend) {
		...
		if err := g.DeleteRelationship(rel.InternalID().SnowflakeID()); err != nil {
			t.Fatalf("DeleteRelationship: %v", err)
		}
		if _, err := g.GetRelationship(rel.InternalID().SnowflakeID()); !errors.Is(err, ErrRelNotFound) {
			t.Fatalf("GetRelationship after delete err = %v, want ErrRelNotFound", err)
		}
		assertRelIDs(t, "RelationshipsByType after rel delete", mustRelationshipsByType(t, g, "OBSERVED", QueryOpts{}), nil)
		assertRelTombstone(t, g, rel.InternalID().SnowflakeID())

		if err := g.DeleteNode(signalNode.InternalID().SnowflakeID()); err != nil {
			t.Fatalf("DeleteNode(signal): %v", err)
		}
		if _, err := g.GetNode(signalNode.InternalID().SnowflakeID()); !errors.Is(err, ErrNodeNotFound) {
			t.Fatalf("GetNode after delete err = %v, want ErrNodeNotFound", err)
		}
		assertNodeIDs(t, "NodesByLabel(Signal) after delete", mustNodesByLabel(t, g, "Signal", QueryOpts{}), nil)
		assertNodeTombstone(t, g, signalNode.InternalID().SnowflakeID())

		cascadeTarget, cascadeRel := addContractSignalRel(t, g, caseNode)
		if err := g.DeleteNode(caseNode.InternalID().SnowflakeID()); err != nil {
			t.Fatalf("DeleteNode(case cascade): %v", err)
		}
		if _, err := g.GetRelationship(cascadeRel.InternalID().SnowflakeID()); !errors.Is(err, ErrRelNotFound) {
			t.Fatalf("GetRelationship cascade rel err = %v, want ErrRelNotFound", err)
		}
		assertNodeTombstone(t, g, caseNode.InternalID().SnowflakeID())
		assertRelTombstone(t, g, cascadeRel.InternalID().SnowflakeID())
		...
		assertIDSet(t, "AllNodeHistoryIDs after deletes", nodeHistoryIDs, []snowflake.ID{
			caseNode.InternalID().SnowflakeID(),
			signalNode.InternalID().SnowflakeID(),
		})
		relHistoryIDs := mustStoreRelHistoryIDs(t, g.store)
		assertIDSet(t, "AllRelHistoryIDs after deletes", relHistoryIDs, []snowflake.ID{
			rel.InternalID().SnowflakeID(),
			cascadeRel.InternalID().SnowflakeID(),
		})
		...
		if _, err := g.GetNode(cascadeTarget.InternalID().SnowflakeID()); err != nil {
...
func TestStoreContract_Pagination(t *testing.T) {
	runStoreContract(t, "pagination", func(t *testing.T, backend storeContractBackend) {
		...
		firstNodes := mustNodesByLabel(t, g, "Case", QueryOpts{Limit: 2})
		assertNodeIDs(t, "NodesByLabel first page", firstNodes, wantNodeIDs[:2])
		secondNodes := mustNodesByLabel(t, g, "Case", QueryOpts{
			Limit: 2,
			After: firstNodes[len(firstNodes)-1].InternalID().SnowflakeID(),
		})
		assertNodeIDs(t, "NodesByLabel second page", secondNodes, wantNodeIDs[2:4])

		allNodeIDs, err := g.store.AllNodeIDs(QueryOpts{Limit: 3})
		if err != nil {
			t.Fatalf("AllNodeIDs first page: %v", err)
		}
		assertIDSetPreserveOrder(t, "AllNodeIDs first page", allNodeIDs, wantNodeIDs[:3])
		allNodeIDs, err = g.store.AllNodeIDs(QueryOpts{After: allNodeIDs[len(allNodeIDs)-1]})
		if err != nil {
		...
		firstRels := mustRelationshipsByType(t, g, "LINK", QueryOpts{Limit: 2})
		assertRelIDs(t, "RelationshipsByType first page", firstRels, wantRelIDs[:2])
		secondRels := mustRelationshipsByType(t, g, "LINK", QueryOpts{
			Limit: 2,
			After: firstRels[len(firstRels)-1].InternalID().SnowflakeID(),
		})
		assertRelIDs(t, "RelationshipsByType second page", secondRels, wantRelIDs[2:4])

		allRelIDs, err := g.store.AllRelIDs(QueryOpts{Limit: 3})
		if err != nil {
			t.Fatalf("AllRelIDs first page: %v", err)
		}
		assertIDSetPreserveOrder(t, "AllRelIDs first page", allRelIDs, wantRelIDs[:3])
		allRelIDs, err = g.store.AllRelIDs(QueryOpts{After: allRelIDs[len(allRelIDs)-1]})
		if err != nil {
...
func TestStoreContract_TemporalFilters(t *testing.T) {
	runStoreContract(t, "temporal filters", func(t *testing.T, backend storeContractBackend) {
		...
		assertNodeIDs(t, "NodesByLabel Case ValidAt=250", mustNodesByLabel(t, g, "Case", QueryOpts{ValidAt: 250}),
			[]snowflake.ID{caseA.InternalID().SnowflakeID(), caseB.InternalID().SnowflakeID()})
		assertNodeIDs(t, "NodesByLabel Case ValidAt=350", mustNodesByLabel(t, g, "Case", QueryOpts{ValidAt: 350}),
			[]snowflake.ID{caseB.InternalID().SnowflakeID()})
		assertNodeIDs(t, "NodesByLabel Case interval", mustNodesByLabel(t, g, "Case", QueryOpts{ValidStart: 250, ValidEnd: 550}),
			[]snowflake.ID{caseA.InternalID().SnowflakeID(), caseB.InternalID().SnowflakeID(), caseC.InternalID().SnowflakeID()})
		assertNodeIDs(t, "AllNodes ValidAt=250", mustAllNodes(t, g, QueryOpts{ValidAt: 250}),
			[]snowflake.ID{caseA.InternalID().SnowflakeID(), caseB.InternalID().SnowflakeID(), signal.InternalID().SnowflakeID()})
		...
		assertRelIDs(t, "RelationshipsByType ValidAt=250", mustRelationshipsByType(t, g, "TEMPORAL_LINK", QueryOpts{ValidAt: 250}),
			[]snowflake.ID{relA.InternalID().SnowflakeID()})
		assertRelIDs(t, "RelationshipsByType interval", mustRelationshipsByType(t, g, "TEMPORAL_LINK", QueryOpts{ValidStart: 250, ValidEnd: 450}),
			[]snowflake.ID{relA.InternalID().SnowflakeID(), relB.InternalID().SnowflakeID()})
		assertRelIDs(t, "AllRelationships ValidAt=450", mustAllRelationships(t, g, QueryOpts{ValidAt: 450}),
			[]snowflake.ID{relB.InternalID().SnowflakeID()})
	})
}
...
func TestStoreContract_Events(t *testing.T) {
	runStoreContract(t, "events", func(t *testing.T, backend storeContractBackend) {
		...
		caseNode, signalNode, rel := addContractCaseSignalRel(t, g)
		if _, err := g.UpdateNode(caseNode.InternalID().SnowflakeID(), map[string]any{"status": "closed"}); err != nil {
			t.Fatalf("UpdateNode: %v", err)
		}
		if _, err := g.UpdateRelationship(rel.InternalID().SnowflakeID(), map[string]any{"weight": int64(2)}); err != nil {
			t.Fatalf("UpdateRelationship: %v", err)
		}
		if err := g.DeleteRelationship(rel.InternalID().SnowflakeID()); err != nil {
			t.Fatalf("DeleteRelationship: %v", err)
		...
		if events[0].Priority != PriorityHigh || events[2].Priority != PriorityHigh {
			t.Fatalf("create event priorities = %v/%v, want PriorityHigh", events[0].Priority, events[2].Priority)
		}
		if events[3].Priority != PriorityNormal || events[4].Priority != PriorityNormal {
			t.Fatalf("update event priorities = %v/%v, want PriorityNormal", events[3].Priority, events[4].Priority)
		}
		if events[5].Priority != PriorityCritical || events[6].Priority != PriorityCritical {
			t.Fatalf("delete event priorities = %v/%v, want PriorityCritical", events[5].Priority, events[6].Priority)
		}
	})
}
...
func TestStoreContract_Stats(t *testing.T) {
	runStoreContract(t, "stats", func(t *testing.T, backend storeContractBackend) {
		...
		caseNode, signalNode, rel := addContractCaseSignalRel(t, g)
		if _, err := g.GetNode(caseNode.InternalID().SnowflakeID()); err != nil {
			t.Fatalf("GetNode: %v", err)
		}
		if _, err := g.GetRelationship(rel.InternalID().SnowflakeID()); err != nil {
...
func assertNodeTombstone(t *testing.T, g *Graph, id snowflake.ID) {
	t.Helper()
...
func assertRelTombstone(t *testing.T, g *Graph, id snowflake.ID) {
	t.Helper()
...
func mustAllNodes(t *testing.T, g *Graph, opts QueryOpts) []*types.Node {
	t.Helper()
...
func mustNodesByLabel(t *testing.T, g *Graph, label string, opts QueryOpts) []*types.Node {
	t.Helper()
...
func mustNodesByLabelAndProperty(t *testing.T, g *Graph, label, key string, value any, opts QueryOpts) []*types.Node {
	t.Helper()
...
func mustAllRelationships(t *testing.T, g *Graph, opts QueryOpts) []*types.Relationship {
	t.Helper()
...
func mustRelationshipsByType(t *testing.T, g *Graph, typeName string, opts QueryOpts) []*types.Relationship {
	t.Helper()
...
func mustOutgoingRelationships(t *testing.T, g *Graph, id snowflake.ID, typeName string) []*types.Relationship {
	t.Helper()
...
func mustIncomingRelationships(t *testing.T, g *Graph, id snowflake.ID, typeName string) []*types.Relationship {
	t.Helper()
...
func mustStoreNodeHistoryIDs(t *testing.T, store Store) []snowflake.ID {
	t.Helper()
...
func mustStoreRelHistoryIDs(t *testing.T, store Store) []snowflake.ID {
	t.Helper()
...
func idsToNodes(t *testing.T, store Store, ids []snowflake.ID) []*types.Node {
	t.Helper()
...
func idsToRels(t *testing.T, store Store, ids []snowflake.ID) []*types.Relationship {
	t.Helper()
...
func nodeIDs(nodes []*types.Node) []snowflake.ID {
	...
	for _, n := range nodes {
		ids = append(ids, n.InternalID().SnowflakeID())
	}
...
func relIDs(rels []*types.Relationship) []snowflake.ID {
	...
	for _, r := range rels {
		ids = append(ids, r.InternalID().SnowflakeID())
	}
	sort.Slice(ids, func(i, j int) bool { return ids[i] < ids[j] })
	return ids
}
